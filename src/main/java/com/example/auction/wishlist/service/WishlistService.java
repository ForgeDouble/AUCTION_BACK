...
//  위시리스트 생성
	...
	public Wishlist createWishlist(WishlistCreateDto dto) {
        ...
        if(product.getUser().getEmail().equals(email)) {
            throw new BadRequestException("SELF_WISHLIST_FORBIDDEN", "본인 상품에는 위시리스트를 추가 할 수 없습니다.");
        }

        boolean exists = wishlistRepository.existsByUser_UserIdAndProduct_ProductId(user.getUserId(), product.getProductId());
        if (exists) {
            throw new InternalErrorException("ALREADY_WISHLIST_EXIST", "이미 위시리스트에 존재합니다.");
        }
...
//  사용자의 위시리스트 목록 조회
    ...
    public Long getWishlistId(Long productId) {
        ...
        if (!productRepository.existsById(productId)) {
            throw new ResourceNotFoundException("상품을 찾을 수 없습니다. productId=" + productId);
        }
...
//  사용자의 위시리스트 삭제
    ...
    public void deleteWishlistById(Long wishlistId) {
        ...
        Wishlist wishlist = wishlistRepository.findByWishlistId(wishlistId)
                .orElseThrow(() ->
                     new InternalErrorException("WISHLIST_NOT_FOUND","해당 위시리스트를 찾을 수 없습니다. wishlistId=" +  wishlistId));
        ...
        if (!wishlist.getUser().getUserId().equals(user.getUserId())) {
            ...
            throw new InternalErrorException("UNAUTHORIZED_ACCESS", "해당 위시리스트를 삭제할 권한이 없습니다.");
        }
